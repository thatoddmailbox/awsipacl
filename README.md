...
Then, from the [Lambda console](https://console.aws.amazon.com/lambda/home), create a new function. Select "Author from scratch", give it a name, and select "Amazon Linux 2023" (OS-only runtime) as the runtime. You can keep architecture as "x86_64".
...
Now, go to the [API Gateway console](https://console.aws.amazon.com/apigateway/home), and build a new HTTP API. Add your Lambda function as an integration, choosing API Gateway version 2.0. Give the API the same name as your function. Set the resource path to "/{proxy+}". Leave the rest of the settings as their defaults.

Next, you need to deploy this code. In a terminal, go to this folder and run `GOOS=linux go build -o bootstrap .`, followed by `zip awsipacl.zip bootstrap`. Then run `aws lambda update-function-code --function-name FUNCTION-NAME-HERE --zip-file fileb://./awsipacl.zip --region REGION`, but replace FUNCTION-NAME-HERE with the name of your Lambda function and REGION with the region it's in.
...
* `zip awsipacl.zip bootstrap`
* `aws lambda update-function-code --function-name FUNCTION-NAME-HERE --zip-file fileb://./awsipacl.zip --region REGION`

Make sure to replace FUNCTION-NAME-HERE with the name of your Lambda function and REGION with its region! Also, note that both the `config.toml` and everything in the `frontend/` folder will be automatically embedded into the binary and uploaded to Lambda.